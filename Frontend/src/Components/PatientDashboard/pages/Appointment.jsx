  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        const response = await axios.get(`/api/appointments/patient/${patientId.id}`);
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-md">
          <ul className="divide-y divide-gray-200">
            {appointments.map((appointment) => (
              <li key={appointment._id} className="px-4 py-4 sm:px-6 hover:bg-gray-50 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    <p className="text-sm font-medium text-gray-900">
                      Appointment with Dr. {appointment.doctor.name || "Doctor Name"}
                    </p>
                  ...
                  <div className="flex space-x-2">
                    ...
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getPaymentBadgeClass(appointment.paymentStatus)}`}>
